...
> 提示： **本章目标**：深入理解Skills本质，学会使用ClawHub技能市场，掌握必装Skills推荐，学习自定义Skills开发和管理技巧。

> ！ **重要提示**：没有Skills的OpenClaw只是1个聊天机器人，有了Skills才能真正成为你的AI助手！

> 【数据】 **Skills 生态概览**：OpenClaw 拥有庞大的 Skills 生态系统，包括：
> - **内置Skills**：52个（随OpenClaw安装，无需额外安装）
...
- [10.0 Skills本质](#100-skills本质ai的操作说明书)
- [10.1 内置Skills](#101-内置skills无需安装)
- [10.2 ClawHub Skills市场](#102-clawhub-skills市场)
- [10.3 核心Skills推荐](#103-核心skills推荐)
- [10.4 Skills管理](#104-skills管理)
- [10.5 自定义Skills开发](#105-自定义skills开发)
...
![img](https://upload.maynor1024.live/file/1771301631361_b3eb7f1a-4905-49a6-bda7-976b1415e213.png)
...
> 提示： **Skills是什么？** Skills是OpenClaw的插件系统，是让AI助手从"能说"到"能做"的关键转变。安装了Skills之后，你的AI助手可以自动浏览网页、收发邮件、操作GitHub、控制智能家居、生成图片、管理任务，甚至控制你的Tesla汽车！
...
OpenClaw自带52个内置Skills，安装后即可使用，无需额外安装。

### 10.1.1 生产力工具

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `1password` | 1Password密码管理 | 安全管理密码和凭证 |
| `apple-notes` | Apple备忘录 | 创建和管理备忘录 |
| `apple-reminders` | Apple提醒事项 | 设置提醒和待办事项 |
| `bear-notes` | Bear笔记 | Markdown笔记管理 |
| `notion` | Notion集成 | 管理Notion数据库和页面 |
| `obsidian` | Obsidian笔记 | 本地知识库管理 |
...
| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `github` | GitHub集成 | 管理仓库、PR、Issues |
| `coding-agent` | 代码助手 | 代码生成和重构 |
| `skill-creator` | Skill创建工具 | 快速创建新Skills |
| `mcporter` | MCP服务器管理 | 管理MCP服务器 |

### 10.1.3 通讯工具

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `discord` | Discord集成 | Discord消息管理 |
| `slack` | Slack集成 | Slack消息和频道管理 |
| `bluebubbles` | BlueBubbles（iMessage） | iMessage消息管理 |
| `imsg` | iMessage集成 | 发送iMessage |
| `himalaya` | 邮件客户端 | 邮件收发管理 |

### 10.1.4 媒体处理

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `video-frames` | 视频帧提取 | 提取视频关键帧 |
| `openai-image-gen` | OpenAI图像生成 | DALL-E图像生成 |
| `openai-whisper` | Whisper语音识别 | 本地语音转文字 |
| `openai-whisper-api` | Whisper API | 云端语音转文字 |
| `camsnap` | 摄像头截图 | 拍照和截图 |
| `gifgrep` | GIF搜索 | 搜索和管理GIF |
| `nano-pdf` | PDF处理 | PDF读取和处理 |
| `songsee` | 音乐识别 | 识别正在播放的音乐 |
| `summarize` | 内容总结 | 总结网页、文章、视频 |

### 10.1.5 智能家居

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `openhue` | Philips Hue灯光控制 | 控制智能灯光 |
| `eightctl` | Eight Sleep控制 | 控制智能床垫 |
| `sonoscli` | Sonos音响控制 | 控制Sonos音响 |

### 10.1.6 系统工具

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `tmux` | Tmux终端管理 | 管理终端会话 |
| `peekaboo` | macOS UI自动化 | macOS界面自动化 |
| `canvas` | Canvas绘图 | 绘制图形和图表 |
| `session-logs` | 会话日志 | 查看对话历史 |
| `weather` | 天气查询 | 查询天气信息 |
| `clawhub` | ClawHub集成 | 管理Skills |

### 10.1.7 其他工具

| Skill | 说明 | 使用场景 |
|-------|------|----------|
| `gemini` | Gemini模型集成 | 使用Gemini模型 |
| `model-usage` | 模型使用统计 | 查看Token使用情况 |
| `healthcheck` | 健康检查 | 系统健康监控 |
| `trello` | Trello集成 | 管理Trello看板 |
| `spotify-player` | Spotify播放器 | 控制Spotify播放 |
| `voice-call` | 语音通话 | 语音通话功能 |
| `nano-banana-pro` | Gemini图像生成 | 使用Gemini生成图像 |

> 提示： **使用建议**：
> 1. 优先使用内置Skills，稳定可靠
> 2. 内置Skills无需安装，开箱即用
> 3. 如果内置Skills满足需求，无需安装第三方Skills
...
**核心价值**：
- 【目标】 **扩展能力**：让OpenClaw能做更多事情
- 【启动】 **快速部署**：一键安装，即刻使用
- 【国外】 **社区驱动**：开发者共享优质Skills
- 🔄 **持续更新**：Skills不断优化升级
- 【包】 **统一管理**：安装、更新、备份一站式
...
# 从URL安装
npx clawhub@latest install https://github.com/user/skill-name
```
**方式3：通过OpenClaw对话安装**
```
...
# 更新指定Skill
npx clawhub@latest update file-search
...
```bash
# 卸载单个Skill
npx clawhub@latest uninstall file-search

# 卸载多个Skills
npx clawhub@latest uninstall file-search note-sync
...
```
![image-20260220203914723](https://upload.maynor1024.live/file/1771591160280_image-20260220203914723.png)
...
```bash
# ！ 重要：clawhub install 一次只能安装一个 Skill，不支持批量安装
...
for skill in "${skills[@]}"; do
    echo "正在安装: $skill"
    npx clawhub@latest install "$skill" || echo "！ $skill 安装失败"
done
```
### 10.2.7 核心Skills详解（2026年必备）

> 提示： **新增内容**：基于1715+个技能生态，精选7大核心技能，每个都经过实战验证。
...
#### 4. Headless Browser (Playwright)——浏览器自动化 【AI】
...
#### 5. Design-Doc-Mermaid——图表自动生成 【数据】
...
#### find-skills + ProactiveAgent——Skills双幻神 🌟🌟
...
**核心Skills 安装命令汇总**：
```bash
# ！ 重要：clawhub install 一次只能安装一个 Skill
# 安装8大核心Skills（包含Skills双幻神）- 使用循环
...
> 提示： **三种安装方式**：ClawHub CLI（推荐）、对话式安装（最简单）、手动安装（高级用户）

### 10.3.1 方法一：ClawHub CLI一键安装（推荐）★
...
# 安装多个技能（逐个安装）
# ！ 注意：clawhub install 一次只能安装一个 Skill
npx clawhub@latest install brave-search
npx clawhub@latest install file-system-manager

# 或使用循环
for skill in brave-search file-system-manager; do
    npx clawhub@latest install "$skill"
done
```
**常用管理命令**：
...
> 提示： **三大场景完整案例**：内容创作、团队协作、智能家居，都有真实数据支撑。

### 10.4.1 场景一：自动化内容创作流程 ✍️
...
> ！ **重要**：Skills虽然强大，但也存在安全风险。必须严格遵循安全原则。
...
> 提示： **创建自定义Skills**：如果现有的1715+个技能无法满足你的特殊需求，可以创建自己的技能。
...
openclaw examples
```
### 10.3.2 Skills文件格式（AgentSkills兼容）
...
# 2. 发布Skills
# ！ 命令可能不可用: openclaw skill publish
...
# 查看下载统计
# ！ 命令可能不可用: openclaw skill stats

# 查看用户反馈
# ！ 命令可能不可用: openclaw skill feedback
```
---

## Skills管理技巧

### 10.4.1 配置覆盖（~/.openclaw/openclaw.json）
...
```
### 10.4.6 远程macOS节点（Linux Gateway）
...
```bash
# 备份所有Skills
# ！ 命令可能不可用: openclaw skill backup

# 备份到指定位置
# ！ 命令可能不可用: openclaw skill backup --output ~/backups/skills.zip
...
```bash
# 从备份恢复
# ！ 命令可能不可用: openclaw skill restore ~/backups/skills-2026-02-11.zip
...
1. 检查Skill是否启用
   # ！ 命令可能不可用: openclaw skill status file-search

2. 查看错误日志
   # ！ 命令可能不可用: openclaw skill logs file-search

3. 检查门控条件
   cat ~/.openclaw/skills/file-search/SKILL.md | grep "requires"

4. 重新安装
   # ！ 命令可能不可用: openclaw skill reinstall file-search
...
1. 查看性能报告
   # ！ 命令可能不可用: openclaw skill perf file-search

2. 检查网络连接
   ping api.example.com

3. 清理缓存
   # ！ 命令可能不可用: openclaw skill cache clear file-search
...
1. 检查配置文件
   # ！ 命令可能不可用: openclaw skill config show file-search

2. 验证环境变量
   echo $GEMINI_API_KEY

3. 重置配置
   # ！ 命令可能不可用: openclaw skill config reset file-search

4. 查看文档
   # ！ 命令可能不可用: openclaw skill docs file-search
...
2. 查看优先级
   # ！ 命令可能不可用: openclaw skill priority
...
> 提示： **本节目标**：学会将第三方API服务封装为Skills，实现AI绘图、数据同步、视频生成、语音合成等功能。

### 10.9.1 为什么需要API服务Skills

**使用场景**：
- 【设计】 **AI绘图**：Banana、Midjourney、DALL-E
- 【说明】 **数据同步**：Notion、Airtable、语雀
- 【脚本】 **视频生成**：SeeDream、可灵（Kling）
...
```bash
# 安装Banana Skills
npx clawhub@latest install banana-draw
...
```bash
# 安装视频生成Skills
npx clawhub@latest install video-gen
...
> 提示： **实战指南**：OpenClaw的Skills系统依赖一些外部工具，本节介绍两个常用工具的完整安装过程。

### 10.10.1 1Password CLI安装（密码管理）
...
**下一章预告**：第11章将学习多平台集成，包括飞书、企业微信、钉钉、QQ等平台的Bot配置，实现随时随地使用 OpenClaw。飞书作为最现代化的平台将优先介绍。
